...
To calculate own and cross price elasticities for each brand we built models with different functional forms (linear, semi-log, and log-log). The dependent variable is `volume` and `price`, `ad` (averstizing) and `promo` (promotion) are the first three independent variables. Then, information about the competition was added to account for their potential effects. For the Semi-Log form we use the log of `volume` as independent variable while the predictors stay the same. For the log-log model, we use the log of `volume` as independent variable as well as log forms for `price`. The general models look as follows:
...
For each model, every combination of predictors was evaluated using a stepwize selection algorithm. The best models for each functional form were then compared by their root mean squared error (RMSE). The model with the lowest RMSE was selected to calculate own and cross-price elasticities.
...
The best model for product Product 2 and Product 6 were the linear models. The best models Product 3 and Product 4 were semi-log and log-log respectively. The formulas used to calculate own and cross-price elasticities given the selected model are as follows:
...
When examining the elasticity matrix, we notice a number of interesing things. The own price elasticites displays a range from -3.020 to -1.99. These results are in line with what is expected in the food FMCGs. Product 4 has the lowest price elasticity at `r round(elasticity.44, 3)`. As the premium brand, its sales are more price sensitive as consumers as they already pay a high price. Product 2 has the highest own price elasticity at `r round(elasticity.22, 3)`. This implies that, within the scope of our analysis, buyers of Product 2 are the least price sensitive. 
